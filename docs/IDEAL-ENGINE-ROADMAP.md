...
```
AF0 ♻️🟢 Catch-rate + tool-selection tabanını genişlet (MathHead)  ✅ v1D: taban + genişleme tek kanıtta — çekirdek yakalama-oranı 613 yanlış-iddialık düşman bataryasında ölçülür (0 ihlal + 3 pozitif kontrol, oran 1.0 deterministik), tool-selection benchmark tabanları canlı koşulur (top-3/top-1 dürüst tabanların üstünde), ve taban DISCOVERY yüzeyine genişler: check() kapısına 126 yanlış-iddialık sistematik batarya (modüler aile + 6 ek yüzeyden bilinen-yanlışlar) beslenir, hiçbiri 'proved' basamaz + X2 haritası en-güçlü aleti önce önerir; kanıt: test_af0_catch_rate_and_tool_selection_extend_to_the_discovery_surface
AF1   🟡 Keşif-oranı metriği: birim başına yeni/doğru/ilginç lemma; novelty-vs-literatür  🟢 evaluation.Scorecard: doğruluk (17/45 verified) + KNOWN sonuçlara atıf (%100 bilinen matematiğe eşlenir) + DÜRÜST novelty: 0 novel-to-literature (motor bilineni yeniden keşfediyor, keşif DEĞİL); raporda HONEST SCORECARD; tam novelty korpus ingest gerektirir (X1/W2, yapılmadı); ADR-D0035  ✅ v1D: metrik kanıtla pinli — birim sayıları (total/verified/attributed) test-yerel bağımsız yüklemlerle yeniden hesaplanıp birebir tutar, iki tamamen taze pipeline koşusu özdeş kart üretir (deterministik), novelty alanı DÜRÜSTÇE 0 ('0 established' + korpus-yok notu; unattributed = aday, iddia değil) ve raporun meta.scorecard'ı canlı kartla alan-alan aynı; kanıt: test_af1_discovery_rate_metric_is_deterministic_and_novelty_is_honestly_zero
AF2   🟢 Regresyon çitleri + deterministik replay (her keşif tekrar-üretilebilir)  ✅ v1D: iki uçtan-uca pipeline koşusu bayt-özdeş markdown üretir, depodaki SAMPLE-REPORT.md canlı motor çıktısıyla bayt-özdeş (regresyon çiti repo'nun içinde yaşar ve gen_status --check + testle çift kilitli), kernel replay aynı teoremi aynı hash'le yeniden basar + check() kapısındaki proof_hash taze kernel koşusundan yeniden türetilir, tohumlu av (frankl) aynı seed'de alan-alan özdeş; kanıt: test_af2_regression_fences_and_deterministic_replay
AF3   🟡 İnsan değerlendirme paneli (ilginçlik ground-truth'u); v1D: kapanamaz — ilginçlik ground-truth'u gerçek İNSAN paneli ister; konteynerde toplanamaz, sentetik/otomatik etiketi panel diye sunmak dürüstlük ihlali olur (🟡 kalır)
```

**Track AG — Altyapı: ölçek, determinizm, güvenlik, CI, docs**
```
AG0 ♻️🟢 Determinizm/seed/replay + provenance/hash (MathHead disiplinini genele taşı)  ✅ v1D: seed-disiplini TÜM stokastik yüzeylerde kanıtla pinli — evolve (adaptive_search.hunt SA, seed alanı sonuçta taşınır), hunt (hunt_frankl + CLI --json çıktısı iki koşuda bayt-özdeş, seed kullanıcıya aynen döner) ve sample (guard_sampled; coverage notu 'proves NOTHING' korunur) aynı seed'de alan-alan özdeş; ürün kapısının tam zarfı (proof_hash dahil) çağrılar-arası sabit; kanıt: test_ag0_seed_discipline_spans_evolve_hunt_sample_and_the_product_door
AG1   🟡 Dağıtık/paralel arama + iş kuyruğu + önbellek + artımlı hesap  ✅ v1D: İN-CONTAINER dilim (dürüst kapsam: tek-makine multiprocessing + yerel disk-önbellek; çok-makine dağıtım/iş-kuyruğu DIŞARIDA ve iddia edilmiyor — SCOPE_NOTE her sonuçta taşınır) — parallel_search.py: sipariş-bölmeli paralel graf-süpürme + paralel Ramsey-karar, işçi sayısı parametre ama SONUÇ işçi sayısından bağımsız (workers=3 == workers=1 alan-alan; tanık = en-küçük-mertebe ihlali, deterministik birleştirme), hash-anahtarlı DiskCache (şema-sürümlü anahtar; bozuk/yabancı/eski-şema girdi = MISS ve yeniden hesap, asla güvenilmez; invalidate() ile açık temizleme; garanti dürüstçe sınırlı: bozulma/bayatlamaya karşı — değerin doğruluğu onu üreten deterministik işçiden gelir) — önbellekli ikinci koşu tamamen hit ve AYNI cevap, max_n üretim-duvarı tek karşılaştırmayla up-front reddedilir (işçi başlatılmadan); kanıt: test_ag1_parallel_sweep_equals_serial_and_the_disk_cache_never_changes_an_answer + test_ag1_parallel_ramsey_decisions_merge_deterministically
AG2 ♻️🟢 Kaynak fençleri + güvenlik/threat-model (güvenilmez girdi, compute bütçesi)  ✅ v1D: fence ENVANTERİ kanıtla pinli — her guard çağrılabilir VE tetiklenebilir (guardrail boyut çiti GUARDRAIL_VIOLATION basar, resource_limits envanteri observability.limits ile birebir, z3 seed=42+timeout=5000ms belgelenen duruşla aynı, üretim duvarları n>7/n!>7/m>4 REDDEDER, geng hard_cap + union_closure cap açık-red, ürün kapısı modulus/4000-hane/oversized refusal'ları), threat-model'e discovery-yüzeyi sympify satırı (T8) eklendi ve satır kodla birebir: product.py sympify kullanır (operatör yüzeyi, MCP aleti DEĞİL), MCP compute yolu allowlist'te kalır; kanıt: test_ag2_resource_fence_inventory_and_threat_model_cover_the_discovery_surface
AG3 ♻️🟢 CI matris/release + paketleme + sürümleme (kernel/DSL/contract)  ✅ v1D: CI matrisi + release + paketleme kanıtla pinli — ci.yml'in beş işi (trackers/test/test-solvers/reproducible/build) ve 3 OS × py3.10-3.12 matrisi + ruff + coverage kapısı + wheel-build/twine + constraints-kurulumu dosyadan doğrulanır, release.yml trusted-publishing (OIDC id-token) + docs.yml deploy pinli, üç konsol girişi (mathhead/mathhead-server/mathhead-discover) pyproject'ten çözümlenip ÇAĞRILABİLİR hedefe bağlanır, sürüm pyproject==__init__ tutarlı (test_release ile çift kilit) ve build-job'un wheel-smoke'unun in-process eşdeğeri koşulur; kanıt: test_ag3_ci_matrix_release_and_packaging_are_pinned
AG4 ♻️🟢 Dokümantasyon + ADR disiplini + kod=docs  ✅ v1D: kod=docs kanıtla pinli — mkdocs nav ↔ docs/manual dosyaları ÇİFT yönlü eksiksiz (ölü nav girişi yok, yetim sayfa yok; araç varsa strict build canlı koşulur, CI docs.yml deploy adımı ayrıca pinli), galeri kapsamı DOCS'TAN türetilir ve çit VERDICTE bağlıdır (quickstart+examples içindeki 18 'mathhead-discover check' komutu çıkarılıp AYNEN çalıştırılır; 'unsupported' düşen komut testi düşürür — CLI unsupported'a artık sıfır-dışı exit 3 döner, stdout zarfı değişmeden — docs çürüyemez), ADR arşiv bütünlüğü: ADR-D0001..D0042 eksiksiz+tekil + donmuş-arşiv başlığı korunur, ana DECISIONS.md numaraları tekil-artan; kanıt: test_ag4_docs_build_gallery_coverage_and_adr_integrity
AG5   🟡 Gözlemlenebilirlik/metrikler + maliyet takibi  ✅ v1D: İN-CONTAINER metrik dilimi (dış telemetri YOK — gizlilik; PRIVACY_NOTE her snapshot'ta taşınır; 'maliyet' = süre + çağrı + solver-çağrı sayacı, bulut faturası kapsam dışı) — instrumentation.py: opsiyonel toplayıcı kapalı-varsayılan ve sonuçları ASLA değiştirmez (observe passthrough), check/bracket/hunt/report op'ları için süre + verdict dağılımı + solver-çağrı sayacı, JSON snapshot/dump yalnız yerel dosyaya, CLI --stats bayrağı invokasyon-özel YEREL Collector ile istatistiği try/finally epiloğunda stderr'e basar (stdout sözleşmesi değişmez; istisna yolunda da yapışma/sızıntı yok, kütüphane-kullanıcısının varsayılan toplayıcısına asla dokunulmaz — iki yönlü izolasyon testte pinli); kanıt: test_ag5_instrumentation_collects_metrics_without_changing_results + test_ag5_cli_stats_flag_reports_to_stderr_and_keeps_the_stdout_contract + test_ag5_cli_stats_epilogue_survives_exceptions_and_never_leaks
```

"""
mathhead.core.inequality — Inequality proving & nonlinear real arithmetic (NRA).

Using Z3's nonlinear real arithmetic (NRA) decision procedure (nlsat / CAD),
PROVES polynomial inequalities or produces a counterexample.

Method: we prove the claim `∀x. P(x)` by checking whether `¬P(x)` is UNSAT
(proof by refutation). SAT → counterexample; UNSAT → valid.

Honesty: NRA is decidable in theory (real closed fields), but in practice Z3 may
return `unknown`/timeout on hard instances — this is reported first-class.

Input grammar (separate from compute, NONLINEAR allowed here): variables are `Real`;
`+ - * / **` (exponent = non-negative integer), comparison `< <= > >= == !=`,
connectives `and`/`or`/`not(...)`/`implies(a,b)`/`iff(a,b)`. NO `eval`/`sympify`.
"""
...
class _IneqError(ValueError):
    """Inequality grammar violation (clear error, no silent assumptions)."""
...
def _node(node: ast.AST, rvars: dict[str, Any]) -> Any:
    ...
    if isinstance(node, ast.UnaryOp):
        ...
        if isinstance(node.op, ast.UAdd):
            return _node(node.operand, rvars)
        raise _IneqError("disallowed unary operator")
    if isinstance(node, ast.BinOp):
        ...
        if isinstance(op, ast.Pow):
            if not (isinstance(node.right, ast.Constant)
                ...
                raise _IneqError("exponent must be a non-negative integer (polynomial)")
            return lft ** node.right.value
        raise _IneqError("disallowed operator")
    if isinstance(node, ast.Compare):
        cur = _node(node.left, rvars)
        parts = []
        for op, comp in zip(node.ops, node.comparators):
            if type(op) not in _CMP:
                raise _IneqError("disallowed comparison")
            rn = _node(comp, rvars)
    ...
    raise _IneqError("could not parse expression")
...
def _real_witness(model: z3.ModelRef, rvars: dict[str, Any]) -> dict[str, Any]:
    """Assigns a concrete point to the real variables (canonical completion → determinism)."""
...
def prove_inequality(goal: str, assumptions: list[str] | None = None,
    ...
    """Does the inequality `goal` hold for ALL real values (under the assumptions)?

    valid → true everywhere (¬goal UNSAT). invalid → `witness` is a counterexample.
    unknown → Z3 NRA could not decide (honest).
    """
...
def prove_nonnegative(expression: str, assumptions: list[str] | None = None,
    ...
    Special case: sum-of-squares-like non-negativity claims (e.g. `x**2 - 2*x + 1`).
    """
...
def find_real_solution(constraints: list[str],
                       timeout_ms: int = DEFAULT_TIMEOUT_MS,
                       seed: int = DEFAULT_SEED) -> ReasoningResult:
    """Finds a point over the REALS satisfying a nonlinear constraint set.

    sat → `witness` is a concrete solution; unsat → no real solution; unknown → no decision.
    """
    t0 = time.perf_counter()
    rvars: dict[str, Any] = {}
    try:
    ...
    if res == z3.sat:
        wit = _real_witness(solver.model(), rvars)
        return ReasoningResult("sat", "MODEL_FOUND", f"real solution found: {wit}.",
                               wit, _meta(t0, seed, timeout_ms))
    if res == z3.unsat:
        return ReasoningResult("unsat", "NO_MODEL", "no real solution (constraints are inconsistent).",
                               None, _meta(t0, seed, timeout_ms))
    return ReasoningResult("unknown", "SOLVER_UNKNOWN",
                           "Z3 NRA could not decide (hard instance / timeout).",
                           None, _meta(t0, seed, timeout_ms))

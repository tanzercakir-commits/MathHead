...
import mathhead.safe_cache as cache
from mathhead.run_audit import RunAuditBundle
from mathhead.safe_cache import (
    SafeCacheDecision,
    SafeCacheEntry,
...
def validators() -> tuple[object, ...]:
    entry_schema = json.loads((SCHEMAS / "safe-cache-entry-v1.schema.json").read_text())
    decision_schema = json.loads((SCHEMAS / "safe-cache-decision-result-v1.schema.json").read_text())
    request_schema = json.loads((SCHEMAS / "safe-cache-request-v1.schema.json").read_text())
    if jsonschema is None or Registry is None or Resource is None:
        return ()
    registry = Registry().with_resource(
        "https://mathhead.dev/schemas/safe-cache-entry-v1.schema.json",
        Resource.from_contents(entry_schema),
    ).with_resource(
        "safe-cache-entry-v1.schema.json", Resource.from_contents(entry_schema)
    )
    return (
...
class SafeCacheContractTests(unittest.TestCase):
    def test_accepted_contract_schema_hashes_and_signature_are_exact(self) -> None:
        self.assertEqual(
            sha((ROOT / "docs/contracts/MH-C-SAFE-CACHE-001.json").read_bytes()),
            cache.CONTRACT_SHA256,
        )
        expected = {
            "mathhead.safe-cache-request.v1": "safe-cache-request-v1.schema.json",
            "mathhead.safe-cache-entry.v1": "safe-cache-entry-v1.schema.json",
            "mathhead.safe-cache-decision-result.v1": "safe-cache-decision-result-v1.schema.json",
        }
        self.assertEqual(
...
class SafeCacheTests(unittest.TestCase):
    ...
    def test_compiled_identity_inventories_are_closed(self) -> None:
        self.assertEqual((len(cache.CONTRACT_BINDINGS), len(cache.SCHEMA_BINDINGS)), (21, 66))
        self.assertEqual((len(cache.IMPLEMENTATION_BINDINGS), len(cache.CONFIGURATION_BINDINGS)), (7, 7))
